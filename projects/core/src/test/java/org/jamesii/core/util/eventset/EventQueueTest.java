...
public abstract class EventQueueTest extends ChattyTestCase {
   ...
   * not identical
   * 
   * TODO: This method does not assert anything and only logs the result of the
   * test, since the event queue's expected behavior is specified in its
   ...
   * {@link Collections#shuffle(List, java.util.Random))} internally and is thus
   * non-deterministic (but should be reproducible if same seed is used).
   * 
   * TODO: This method does not assert anything and only logs the result of the
   * test, since the event queue's expected behavior is specified in its
   * factory, not the event queue class itself.
  ...
  public final void testEventOrderingBehavior() {
    ...
    final long offset = Long.MAX_VALUE / 2;
    NavigableMap<Double, List<Long>> enqOrders =
        addSameTimeEventsToQueues(eq1, eq2, getRandom(), blockAmount,
            blockSize, offset);
    EventOrderingBehavior orderBehavior =
        checkOrderBehavior(eq1, eq2, enqOrders, offset);
   ...
   *          Second event queue
   * @param rand
   *          Random number generator (for shuffling time and event ID order)
   * @param blockAmount
   *          Number of different time stamps
   * @param blockSize
   *          Number of events with each time stamp
   * @param offset
   *          Offset for IDs added to second EQ compared to first one
  ...
  private NavigableMap<Double, List<Long>> addSameTimeEventsToQueues(
    ...
    Collections.shuffle(timeStamps, rnd);

    int numIDs = blockAmount * blockSize;
    long l = 0;
    long step = Long.MAX_VALUE / (numIDs + 1);
    // use small and large long values (from 0 to close to max) as event IDs
    // such that the order of the hash codes and the order of the values is
    // not the same (otherwise NATURAL order would be detected too often)
    List<Long> allEventIDs = new ArrayList<>(numIDs);
    for (int i = 0; i < numIDs; i++) {
      allEventIDs.add(l);
      l += step;
    }
    Collections.shuffle(allEventIDs, rnd);
    // enqueue #blockSize events with the same time stamp for #blockAmount
    // time stamps
    for (int i = 0; i < blockAmount; i++) {
      Double timeStamp = timeStamps.get(i);
      List<Long> eventIDs =
          allEventIDs.subList(i * blockSize, (i + 1) * blockSize - 1);
      enqOrder.put(timeStamp, eventIDs);
      for (Long id : eventIDs) {
        eq1.enqueue(id, timeStamp);
        eq2.enqueue(id + offset, timeStamp);
      }
    }
    return enqOrder;
  }
  ...
  private EventOrderingBehavior checkOrderBehavior(
      IEventQueue<Object, Double> eq1, IEventQueue<Object, Double> eq2,
      NavigableMap<Double, List<Long>> enqOrders, long offset) {
    int numBlocks = 0;
    int fifos = 0;
    int lifos = 0;
    // int lexicals = 0;
    ...
    while (!eq1.isEmpty() && !eq2.isEmpty()) {
      ...
      assertEquals(dequeueAll1.size(), dequeueAll2.size());

      List<Long> deq2minusOffset = new ArrayList<>(dequeueAll2.size());
      for (Object d2 : dequeueAll2) {
        deq2minusOffset.add(((Long) d2) - offset);
      }
      List<Long> enqOrder = enqOrders.get(time1);

      assertEquals(enqOrder.size(), dequeueAll1.size());
      // System.err.println(eq1.getClass().getSimpleName()
      // + " order test, time stamp " + time1 + " (block" + numBlocks
      // + ")\nEnqueue order   :" + enqOrder + "\nDequeue order 1 :"
      // + dequeueAll1 + "\nDequeue order 2 :" + deq2minusOffset
      // + (offset != 0 ? "\n(without offset):" + dequeueAll2 : ""));
      if (!dequeueAll1.equals(deq2minusOffset)) {
    ...
    assertTrue("One eq is not empty (not an ordering problem)", eq1.isEmpty()
        && eq2.isEmpty());
    if (numBlocks == fifos) {
