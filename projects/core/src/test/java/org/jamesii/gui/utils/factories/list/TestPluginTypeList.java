...
import org.jamesii.gui.utils.list.view.item.PluginTypeItem;

public class TestPluginTypeList {
  ...
  public static void main(String[] args) {
    ...
    List<IPluginTypeData> infos = SimSystem.getRegistry().getPluginTypes();

    IViewableItem[] items = new IViewableItem[infos.size()];
    int c = 0;
    for (IPluginTypeData pdata : infos) {
      items[c] = new PluginTypeItem(pdata);
      c++;
    }
